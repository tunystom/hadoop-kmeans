...
public class KMeans extends Configured implements Tool
{
    ...
    {
        System.exit(ToolRunner.run(new KMeans(), arguments));
    }

    @Override public int run(String[] arguments) throws Exception
    {
        ArgumentParser parser = new ArgumentParser("KMeans");
        ...
        parser.addArgument("t", "iterations", true, "500", "NUM", false, "specify maximum number of iterations");
        ...
        else if (parser.hasOption("k"))
        {
            ...
            RandomVectorSelector.run(inputPath, temporaryInputPath, parser.getInt("k"), getConf(), verbose);
        }
        else
        {
    ...
    {
        // Create job.
        Job job = new Job(getConf(), "KMeansJob");
        ...
        // Put the location of the cluster centers into job's configuration.
        job.getConfiguration().set("cluster.centers.input.path", centersInputPath.toString());
     ...
     */
    public static int getVectorDimensionality(Path inputPath, Configuration conf) throws IOException
    {
    ...
    private static ArrayList<VectorWritable> loadCentroidsFromFile(FileSystem fs, Path inputPath, Configuration conf) throws IOException
    {
    ...
    private double updateAndSaveCentroids(Path oldCentroidsPath, Path newCentroidsPath, Configuration conf) throws IOException
    {
    ...
    public int loadApplicationProperties(Path propInputPath) throws IOException
    {
    ...
    public void saveApplicationProperties(Path propOutputPath, int failedIteration) throws IOException
    {
        Properties properties = new Properties();
    ...
    private static class KMeansMapper extends Mapper<LongWritable, VectorWritable, LongWritable, VectorWritable>
    {
        private Path centersPath;
        private Algebra algebra = new Algebra();
                
        @Override protected void setup(Context context) throws IOException, InterruptedException
        {
            centersPath = new Path(context.getConfiguration().get("cluster.centers.input.path"));
        }
        ...
        public void map(LongWritable key, VectorWritable value, Context context) throws IOException, InterruptedException
        {
            ...
            LongWritable bestCenter = new LongWritable(-1);
            double bestDistance = Double.POSITIVE_INFINITY;
            
            SequenceFileReader reader = new SequenceFileReader(centersPath, context.getConfiguration());
            
            LongWritable centerId = new LongWritable();
            VectorWritable centerVector = new VectorWritable();
            
            while (reader.next(centerId, centerVector))
            {
                double distance = 1.0 - algebra.mult(vector, centerVector.getVector());

                if (distance < bestDistance)
                {
                    bestCenter.set(centerId.get());
                    bestDistance = distance;
                }
            }
            
            context.write(bestCenter, value);
        }
    }
    
    private static class KMeansReducer extends Reducer<LongWritable, VectorWritable, LongWritable, VectorWritable>
    {
